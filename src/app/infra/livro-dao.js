  ...
  lista () {
    return new Promise((resolve, reject) => {
      this._db.all('SELECT * FROM livros', (error, resultado) => {
        if (error) {
          return reject(new Error('Não foi possível listar os livros'))
        }
  ...
  adiciona (livro) {
    return new Promise((resolve, reject) => {
      this._db.run(`INSERT INTO LIVROS (titulo, preco, descricao) values (?, ?, ?)`,
        ...
        error => {
          if (error) {
            return reject(new Error(`Não foi possível adicionar o livro --> ${livro}`))
          }
          return resolve()
        })
    })
  ...
  edita (livro) {
    return new Promise((resolve, reject) => {
      this._db.run(`UPDATE livros SET titulo =  ?, preco = ?, descricao = ? WHERE id = ?`,
        ...
        error => {
          if (error) {
            return reject(new Error(`Não foi possível editar o livro --> ${livro}`))
          }
          return resolve()
        })
    })
  ...
  remove (livroId) {
    return new Promise((resolve, reject) => {
      this._db.run(`DELETE FROM livros where id =? `,
        ...
        (error) => {
          if (error) {
            return reject(new Error(`Não foi possível remover o livro --> ${livroId}`))
          }
          return resolve()
        })
    })
